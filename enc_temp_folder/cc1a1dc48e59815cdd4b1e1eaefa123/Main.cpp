...
void adjustDownward() {
    ...
    glm::vec2 direction = models.at(1)->getGravityDirection(references);
    ...
    std::shared_ptr<VertexData> player = models.at(1)->getVertexDataPointer();
    for (int i = 2; i < models.size(); i++) {
        std::shared_ptr<VertexData> temp =models.at(i)->getVertexDataPointer();
        float xDiff = temp->getAvgX() - player->getAvgX();
        float yDiff = temp->getAvgY() - player->getAvgY();
        float angle2 = atan2(yDiff, xDiff);
        float magnitude = sqrt(pow(yDiff, 2) + pow(xDiff, 2));
